                            ...
                            return (
                                <div key={eq.id} className={`rounded-2xl border bg-white dark:bg-slate-900 p-5 shadow-xs flex flex-col justify-between space-y-4 ${
                                    ...
                                    <div>
                                        ...
                                        {eq.fuel_efficiency && (
                                            <div className="mt-2.5 px-3 py-1.5 rounded-xl bg-teal-50 dark:bg-teal-950/40 border border-teal-200 dark:border-teal-800/60 flex items-center justify-between text-xs">
                                                <span className="flex items-center gap-1.5 text-[11px] font-bold text-teal-700 dark:text-teal-300">
                                                    <Fuel className="w-3.5 h-3.5" />
                                                    <span>Fuel Burn Rate</span>
                                                </span>
                                                <span className="font-mono font-black text-teal-800 dark:text-teal-200 text-xs">
                                                    {eq.fuel_efficiency} L/hr
                                                </span>
                                            </div>
                                        )}
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
                        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 w-full max-w-lg space-y-4">
                            ...
                            <form onSubmit={handleLogSubmit} className="space-y-4">
                                ...
                                {logForm.data.log_type === 'fuel' && (
                                    <div>
                                        <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">
                                            Fuel Quantity Added (Liters) <span className="text-rose-500">*</span>
                                        </label>
                                        <input
                                            type="number"
                                            step="0.1"
                                            min="0"
                                            required
                                            value={logForm.data.fuel_liters}
                                            onChange={(e) => logForm.setData('fuel_liters', e.target.value)}
