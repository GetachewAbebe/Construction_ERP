...
import { Save, ArrowLeft, Building2, Briefcase, Mail, Phone, Calendar, DollarSign, Image } from 'lucide-react';
...
export default function Edit({ employee, departments = [], positions = [] }) {
    const { data, setData, post, processing, errors } = useForm({
        ...
        department_name: employee.department_rel?.name || employee.department || '',
        position_title: employee.position_rel?.title || employee.position || '',
        profile_picture: null,
    });

    const handleSubmit = (e) => {
        e.preventDefault();
        post(`/hr/employees/${employee.id}`);
    };

    return (
        <AuthenticatedLayout title={`Edit ${employee.first_name} ${employee.last_name}`} header="Human Resources">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-xl font-extrabold tracking-tight text-slate-900 dark:text-white">
                                Update Employee: {employee.first_name} {employee.last_name}
                            </h1>
                            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                                Modify workforce details, designations, and status for ID #{employee.employee_id || employee.id}.
                            </p>
                        </div>
                    </div>
                </div>
