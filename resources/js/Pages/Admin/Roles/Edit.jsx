...
export default function Edit({ role, permissions = {}, rolePermissions = [] }) {
    ...
    return (
        <AuthenticatedLayout title={`Edit Role • ${role.name}`} header="System Administration">
                        ...
                        <div>
                            <div className="flex items-center gap-2">
                                <h1 className="text-xl sm:text-2xl font-extrabold tracking-tight text-slate-900 dark:text-white">
                                    Reconfigure Role: {role.name}
                                </h1>
                                {isProtected && (
                                    <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-blue-100 dark:bg-blue-900/40 text-blue-800 dark:text-blue-300">
                                        <Lock className="w-3 h-3" /> Core System Tier
                                    </span>
                                )}
                            </div>
                            <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400">
                                Update privilege matrix and access boundaries for this security group.
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm space-y-5">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input
                                    type="text"
                                    placeholder="e.g. Finance Operations Manager"
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm space-y-5">
                        <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-800">
                            <div className="flex items-center gap-2 text-slate-900 dark:text-white font-bold text-sm">
                                ...
                                <span>Privilege Matrix ({data.permissions.length} Authorized)</span>
                            </div>
                        </div>
                    ...
                    <div className="flex items-center justify-end gap-3 pt-2">
                        ...
                        >
                            ...
                            <span>{processing ? 'Saving Changes...' : 'Save Role Matrix'}</span>
                        </button>
                    </div>
                </form>
            </div>
