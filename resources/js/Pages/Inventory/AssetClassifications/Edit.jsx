...
export default function Edit({ classification, parents = [] }) {
    const { data, setData, put, processing, errors } = useForm({
        name: classification.name || '',
        code: classification.code || '',
        parent_id: classification.parent_id || '',
        icon_identifier: classification.icon_identifier || '',
        description: classification.description || '',
    });

    const handleSubmit = (e) => {
        e.preventDefault();
        put(`/inventory/asset-classifications/${classification.id}`);
    };

    return (
        <AuthenticatedLayout title={`Edit • ${classification.name}`} header="Inventory & Store">
            <Head title={`Edit Category: ${classification.name}`} />
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <div>
                            <h1 className="text-xl sm:text-2xl font-extrabold tracking-tight text-slate-900 dark:text-white">
                                Edit Category: {classification.name}
                            </h1>
                            <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400">
                                Hierarchy code: <span className="font-mono font-semibold">{classification.code}</span>
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm space-y-4">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">
                                    Nomenclature Code <span className="text-rose-500">*</span>
                                </label>
                                <input
                                    type="text"
                                    required
                    ...
                    <div className="flex items-center justify-end gap-3">
                        ...
                        >
                            ...
                            <span>{processing ? 'Saving...' : 'Update Category'}</span>
                        </button>
                    </div>
                </form>
            </div>
