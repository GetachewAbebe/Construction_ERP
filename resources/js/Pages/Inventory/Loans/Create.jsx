...
export default function Create({ items = [], employees = [] }) {
    ...
    const selectedItem = items.find(i => String(i.id) === String(data.inventory_item_id));

    return (
        <AuthenticatedLayout title="Issue Material Loan" header="Inventory & Store">
            <div className="max-w-2xl mx-auto space-y-6">
                <div className="flex items-center justify-between pb-2 border-b border-slate-200 dark:border-slate-800">
                    <div>
                        <h1 className="text-xl sm:text-2xl font-extrabold tracking-tight text-slate-900 dark:text-white">
                            Issue Material Loan / Gate Pass
                        </h1>
                        <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-0.5">
                            Authorize site personnel to checkout tools or machinery with an active gate pass.
                        </p>
                    </div>
                ...
                <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 sm:p-8 shadow-xs">
                    <form onSubmit={submit} className="space-y-4">
                        ...
                        <div>
                            <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wider">
                                Stock Item To Checkout <span className="text-rose-500">*</span>
                            </label>
                            <select
                                required
                            ...
                            >
                                ...
                                {items.map((it) => (
                                    <option key={it.id} value={it.id}>
                                        {it.name} (Stock: {it.quantity} {it.unit_of_measurement || 'pcs'} - {it.store_location || 'Warehouse'})
                                    </option>
                                ))}
                            </select>
                            {selectedItem && (
                                <p className="text-[11px] text-slate-500 mt-1">
                                    Available: <strong>{selectedItem.quantity} {selectedItem.unit_of_measurement || 'pcs'}</strong> in <strong>{selectedItem.store_location || 'Depot'}</strong>
                                </p>
                            )}
                        </div>
                        ...
                        <div>
                            <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wider">
                                Loan Purpose & Project Site Details
                            </label>
                            <textarea
                                rows={3}
                                placeholder="Site location, project milestone or task justification..."
                                value={data.notes}
                        ...
                        <div className="pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-end gap-3">
                            ...
                            >
                                ...
                                <span>{processing ? 'Processing...' : 'Issue Gate Pass'}</span>
                            </button>
                        </div>
                    </form>
                </div>
