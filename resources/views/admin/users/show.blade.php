@extends('layouts.app')
@section('title', $user->name . ' - Professional Identity')

@section('content')
<div class="row align-items-center mb-4 stagger-entrance">
    <div class="col">
        <h1 class="h3 mb-1 fw-800 text-erp-deep">{{ $user->role ?? 'User' }} Profile</h1>
        <p class="text-muted mb-0">Reviewing professional anchoring for: <span class="text-erp-deep fw-800">{{ $user->name }}</span></p>
    </div>
    <div class="col-auto">
        <a href="{{ route('admin.users.index') }}" class="btn btn-white rounded-pill px-4 shadow-sm border-0 transition-all transform-hover">
            ...
            <div class="p-5 bg-white">
                ...
                <div class="mt-5 pt-5 border-top border-light d-flex justify-content-between align-items-center">
                    <div class="text-muted small fw-bold text-uppercase tracking-widest">
                        <i class="bi bi-clock-history me-2"></i>Last Administrative Registry Update: {{ $user->updated_at ? $user->updated_at->format('M d, Y') : 'Original Record' }}
                    </div>
