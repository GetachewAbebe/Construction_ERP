@extends('layouts.app')
@section('title', 'Reconfigure Asset Classification')

@section('content')
<div class="page-header-premium mb-4">
    <div class="row align-items-center">
        <div class="col">
            <h1 class="display-6">Edit Category: {{ $itemCategory->name }}</h1>
            <p>Update metadata and structural mapping for the '{{ $itemCategory->name }}' classification.</p>
        </div>
        <div class="col-auto">
...
<div class="row justify-content-center stagger-entrance">
    <div class="col-lg-8">
        <div class="erp-card shadow-lg border-0">
            <form action="{{ route('admin.item-categories.update', $itemCategory) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="row g-4">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="parent_id" id="parent_id" class="btn btn-white w-100 text-start border py-3 shadow-none @error('parent_id') is-invalid @enderror" style="border-radius: 12px;">
                            ...
                            @foreach($parentCategories as $parent)
                                <option value="{{ $parent->id }}" {{ old('parent_id', $itemCategory->parent_id) == $parent->id ? 'selected' : '' }}>
                                    {{ $parent->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="input-group">
                            <span class="input-group-text bg-light border-0 px-4" style="border-radius: 12px 0 0 12px;">
                                <i class="bi {{ $itemCategory->icon ?: 'bi-box-seam' }}"></i>
                            </span>
                            <input type="text" name="icon" id="icon" value="{{ old('icon', $itemCategory->icon) }}" 
                    ...
                    <div class="col-12">
                        ...
                        <textarea name="description" id="description" rows="4" 
                                  ...
                                  placeholder="Describe the scope and purpose of this classification tier...">{{ old('description', $itemCategory->description) }}</textarea>
                        @error('description')
                            <div class="invalid-feedback fw-bold">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="mt-5 border-top pt-4 text-end">
                    <button type="submit" class="btn btn-erp-deep rounded-pill px-5 py-3 shadow-sm border-0 fw-800">
                        <i class="bi bi-shield-check me-2"></i>Update Classification
                    </button>
                </div>
            </form>
        </div>
        ...
        @if($itemCategory->items()->count() > 0)
            <div class="alert alert-info mt-4 border-0 shadow-sm rounded-4 stagger-entrance" style="background: rgba(13, 110, 253, 0.05); color: #084298;">
                <div class="d-flex align-items-center gap-3">
                    <i class="bi bi-info-circle-fill display-6"></i>
                    <div>
                        <h6 class="fw-800 mb-1">Structural Notice</h6>
                        <p class="mb-0 small fw-bold">This classification is currently linked to <strong>{{ $itemCategory->items_count }}</strong> inventory assets. Structural changes will propagate across all linked registry entries.</p>
                    </div>
                </div>
            </div>
        @endif
